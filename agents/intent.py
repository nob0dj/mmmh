...
llm = ChatOpenAI(
    ...
    model_kwargs={                        # OpenAI에 전달할 추가 옵션
        "response_format": {"type": "json_object"}  # 기본값 {"type": "text"} (최근 변경사항)
    }
)

# 함수명, docstring이 Node명, Description으로 사용된다. 
def classify_intent(state: dict) -> dict:
    ...
    # - system instruction(persona 기술) : 별도의 SystemMessage로 작성할 수 도 있고, UserMessage 첫줄에 작성해도 무방하다.
    # - context
    # - input indicator
    # - output indicator
    # 
    # 실제로는 food/activity(양성) 또는 unknown(음성)을 분류하는 것이므로 다음 한줄이 아주 효과적일수 있다. 
